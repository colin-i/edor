...
\nCtrl+j = enable/disable OA split syntax; Alt+j = change delimiter; Alt+J = change escape delimiter; Alt+o = splits folder\
\nCtrl+q = quit"
...
static bool loopin(WINDOW*w){
	...
	for(;;){
		...
		if(a!=0){
			...
			else if(z=='o'){if(pref_change(w,&split_out,&split_out_new,false)/*true*/)return true;}//Alt+S is nothing here
